...
# 15/02/2018 add command for openjabnab.fr Thaichi on rfid (TA go)
import random
import string
import md5
import subprocess
...
##    createSock
    ...
    TA=0 # 15/02/2018 add 
    while 1:
        if infoTaichi!='00' or TA==1: # 15/02/2018 add 
          now=int(time.time())
          if now>(times+nextTaichi) and sleep==0 or TA==1: # 15/02/2018 add
              TA=0
...
##            except socket.timeout, e: #v4            
    ...
    #add type for openjabnab.fr !!!
            ...
            for el in newlistn:
                sound=''  
                mm = re.search('TA (.+)', el) # 15/02/2018 add
                if mm: # 15/02/2018 add
                  TA=1 # 15/02/2018 add
